...
class TestConfigurationBuilder implements ConfigurationBuilderInterface
{
    ...
    public function build($name)
    {
        ...
        $config = self::merge($config, $sections);
        $env = $this->buildEnv($name, ConfigKit::env()->get('YII_ENV','test'));

        $config = self::merge($config, $env);
    ...
    protected function buildEnv($name, $env)
    {
        ...
        $local = ConfigKit::str()->is($env, 'local')
            ...
            : self::merge($sections, $this->buildEnv($name, 'local'));
        return self::merge($config, $local);
    }
    /**
     * Caches config file.
     *
